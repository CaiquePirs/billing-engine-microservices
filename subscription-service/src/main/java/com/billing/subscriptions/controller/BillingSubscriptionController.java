...
public class BillingSubscriptionController {
    ...
    @PreAuthorize("hasAuthority('ROLE_CUSTOMER')")
    public ResponseEntity<BillingSubscriptionResponseDTO> getSubscription(@PathVariable UUID subscriptionId) {
        BillingSubscription subscription = billingSubscriptionService.findOwnedSubscription(subscriptionId);
        return ResponseEntity.ok(billingSubscriptionMapper.toResponse(subscription));
    }
}
